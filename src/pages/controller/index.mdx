...
Controller is a static class that handles incoming HTTP requests. The methods of this class, that are decorated with an HTTP decorator (like `@get()` or `@post()`), accept 2 arguments: `NextRequest` that is not modified in any way by Vovk.ts itself and parameters that are defined by the decorator path. 
...
Since the client is generated automatically, if your API is used only internally, you can use auto-generated endpoints to save a little bit of time.
