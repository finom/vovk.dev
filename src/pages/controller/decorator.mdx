...
See also:
- [Validation](/validation) - validation with different libraries.
...
That's a simple example of how to use a decorator to add OpenAPI metadata to the handler. The `openapi` decorator adds the OpenAPI operation object to the handler schema. The `OpenAPIController` class generates the OpenAPI specification based on the schema of the handlers. The first argument of `createDecorator` is `null` because the decorator doesn't need to access the request object. Thanks to this, it can be used both below the HTTP decorator and above it.

```ts filename="/src/decorators/openapi.ts"
import type { OperationObject } from 'openapi3-ts/oas31';
import { createDecorator } from 'vovk';

const openapi = createDecorator(null, (openAPIOperationObject: OperationObject = {}) => {
  return (handlerMetadata) => ({
    custom: {
      ...handlerMetadata?.custom,
      openAPIOperationObject: {
        ...(handlerMetadata?.custom?.openAPIOperationObject as object),
        ...openAPIOperationObject,
      },
    },
  });
...
Retrieve the OpenAPI specification from the schema and build the OpenAPI object. At this case the OpenAPI object is going to beb served from the `/api` path as an example of the root endpoint.


```ts filename="/src/controllers/openapi/OpenAPIController.ts"
import type { OpenAPIObject, OperationObject } from 'openapi3-ts/oas31';
import { get, type HttpMethod } from 'vovk';
import vovkSchema from '../../../.vovk-schema';

export default class OpenAPIController {
  @get() // The default path is '/'
  static getOpenAPISpec() {
    const paths: Exclude<OpenAPIObject['paths'], undefined> = {};
    ...
    for (const [segmentName, schema] of Object.entries(vovkSchema)) {
      for (const c of Object.values(schema.controllers)) {
        for (const h of Object.values(c.handlers)) {
          const path = '/' + ['api', segmentName, c.prefix, h.path].filter(Boolean).join('/');
          paths[path] = paths[path] ?? {};
          const openAPIOperationObject = h.custom?.openAPIOperationObject as OperationObject | undefined;
          paths[path][h.httpMethod.toLowerCase() as Lowercase<HttpMethod>] = {
            responses: {
              '200': { description: 'Successful response' },
            },
            ...(h.custom?.openAPIOperationObject as OperationObject),
          };
        }
      }
    }
...
const authGuard = createDecorator(
  async (req, next, permission: Permission) => {
    ...
    try {
      isAuthorized = await checkAuth(req, permission);
    } catch (error) {
      throw new HttpException(HttpStatus.UNAUTHORIZED, `Auth check failed. ${(error as Error).message}`);
    }
    if (!isAuthorized) {
      throw new HttpException(HttpStatus.UNAUTHORIZED, 'Unauthorized');
    }
