...
- Detect JSON stream response by `x-vovk-stream` header and use `text/plain; charset=utf-8` content-type header.
...
- Support **.config** folder for config.
...
- Get rid of `VOVK_PORT` variable because it isn't needed anymore because of the new way how [vovk dev](/cli/vovk-dev) works.
