...
[vovk dev](/cli/vovk-dev) emits JSON files that contain information about endpoints, method names, validation, etc, defined at the controller to **.vovk-schema/** folder. Each **.json** file represents schema for one [segment](/segment). The schemas exported from auto-generated **.vovk-schema/index.js** and **.vovk-schema/index.d.ts**, that are modified when segments are added or removed. 

```js filename=".vovk-schema/index.js"
// auto-generated
...
```ts filename=".vovk-schema/index.d.ts"
// auto-generated
...
As you can see, the type of the schema is defined by `VovkSchema` type that's exported from the **vovk** package. As result, the full schema type is going to look like `Record<string, VovkSchema>`.

Example of the schema file:

```json filename=".vovk-schema/foo.json"
{
  ...
  "workers": {
    "HelloWPC": {
      ...
      "handlers": {
        "factorize": {
            // is the worker method a generator
            "isGenerator": true
        }
      }
    }
