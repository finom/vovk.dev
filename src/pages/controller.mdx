...
Methods of this class, that are decorated with an HTTP decorator (like `@get()` or `@post()`), accept 2 arguments: `VovkRequest` (that is inherited from `NextRequest`) and parameters that are defined by the decorator path. 
...
Once the controller is defined it needs to be initialized at the wildcard route (we call it "segment") by adding it to the `controllers` object. The key is the name of the resulting library exported from `"vovk-client"`. Read more about it in the [Client](/client) section.
