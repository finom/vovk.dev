...
[Procedures](./procedure), being a wrapper over Next.js route handlers, return `Response` objects. Alternatively, they can return JSON objects and HTTP exceptions, that internally get converted to `Response` objects.
...
export default class HelloController {
  ...
  static helloWorld() {
    ...
    }) as unknown as { hello: string }; // for RPC type inference
  }
}
```
