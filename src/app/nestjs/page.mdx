...
You can optionally run `start:dev` and `generate` scripts in parallel using [concurrently](https://www.npmjs.com/package/concurrently) package. The `--watch` option makes `generate` command to watch for changes in the OpenAPI spec and regenerate the client automatically.
...
For more flexibility it's recommended to create a [config](/config) file.
...
In this case, the generated client methods will validate input data with DTOs using [class-validator](https://www.npmjs.com/package/class-validator) library before sending the request to the server.
