...
> **📘 About this example:** While it's called "Hello World," it's intentionally comprehensive—showcasing validation, streaming, multi-language clients, and OpenAPI generation in a single project. If you're looking for the absolute simplest starting point, see the [Quick Start](/quick-install) guide first.

The "Hello World" app, available at [hello-world.vovk.dev](https://hello-world.vovk.dev/) is a Next.js/Vovk.ts example project that demonstrates core Vovk.ts features by implementing the following:

- Back-end:
  - `UserController` with an `updateUser` method exposed as POST at the `/api/users/{id}` endpoint.
  - `StreamController` with a JSONLines handler `streamTokens` exposed as GET at the `/api/streams/tokens` endpoint.
  - `OpenApiController` with `getSpec` exposed as GET at the `/api/static/openapi/spec.json` endpoint, exposing the OpenAPI specification from a static segment. The OpenAPI documentation is available at the [`/openapi` page](https://hello-world.vovk.dev/openapi).
- Front-end:
  - A form component with a JSONLines streaming demo above it.
- A configuration file that enables client-side validation; generates ready-to-publish Rust and Python clients; produces a segmented TypeScript client and an npm bundle; and defines OpenAPI info and servers sections.

The project can be explored in the [GitHub repository](https://github.com/finom/vovk-hello-world). All generated files are committed to Git under [dist](https://github.com/finom/vovk-hello-world/tree/main/dist), [tmp_prebundle](https://github.com/finom/vovk-hello-world/tree/main/tmp_prebundle), [dist_rust](https://github.com/finom/vovk-hello-world/tree/main/dist_rust), and [dist_python](https://github.com/finom/vovk-hello-world/tree/main/dist_python) for exploration (in a real project, they should be ignored via `.gitignore`).

All code snippets on this page are pulled directly from the GitHub source, and example pages are embedded via an iframe.
...
- [Zod validation](/validation/zod) with `body`, `query`, and `params` inputs, `output` and `iteration` outputs, defined with descriptions and example values using Zod's [meta](https://zod.dev/metadata#meta) method.
...
- [Type inference](/inference) in a [service](/controller/service), with the [controller](/controller) method returning the service result.
- Usage of `useQuery` and `useMutation` with the [queryKey](/typescript#react-query) method.
- TypeScript client [bundle](/bundle) with a README, published on [npm](https://www.npmjs.com/package/vovk-hello-world).
- [Generation](/codegen) of experimental [Rust](/rust) and [Python](/python) clients with READMEs, published on [crates.io](https://crates.io/crates/vovk_hello_world) and [PyPI](https://pypi.org/project/vovk-hello-world/).
- [OpenAPI spec](/openapi) with code samples, served from a [static](/segment/static) [segment](/segment/introduction) and rendered using [Scalar](https://scalar.com/).


## Live Demo

The demo implements a simple form (without validation attributes) and a “Disable client-side input validation” checkbox that toggles the [disableClientValidation](/typescript#disableclientvalidation) option. “Notification type” is intentionally set to an invalid value to demonstrate both server- and client-side validation error handling. 

<IFrame src="https://hello-world.vovk.dev" width="100%" height="780px"></IFrame>

## `UserController` and `UserService`

The `/api/users/{id}` endpoint is implemented by `UserController` and `UserService`, which provide the `updateUser` method.

The controller’s `updateUser` method uses the `@post` decorator to map the method to the POST HTTP verb and the `withZod` function to apply [Zod validation](/validation/zod) to the request inputs (`body`, `params`, and `query`). The input Zod models use the [meta](https://zod.dev/metadata?id=meta) method to provide additional OpenAPI details such as `description` and `examples`.

For demonstration purposes, the `body` model performs nested validation for `email` and `profile`, where `profile` is an object with `name` and `age`.

The service `updateUser` method infers its input type from the controller method, which returns the result of the service’s `updateUser` call—avoiding self-reference (implicit “any”) issues.
...
The `/api/streams/tokens` endpoint is implemented by `StreamController` and `StreamService`, which provide the `streamTokens` method.

The controller’s `streamTokens` method uses the `@get` decorator to map the method to the GET verb and the `withZod` function to validate each iteration of the [JSONLines](/controller/jsonlines) response. It delegates iteration using `yield*` to the service’s `streamTokens` generator, which yields tokens one by one and uses `setTimeout` to simulate delays between emissions.
...
The demo components use [@tanstack/react-query](https://www.npmjs.com/package/@tanstack/react-query) for data fetching and streaming.
...
- Enable client-side validation with [Ajv](/validation/client).
- Automatically generate [Rust](/rust) and [Python](/python) code when running [vovk dev](/dev) or [vovk generate](/generate).
...
The [OpenAPI specification](https://hello-world.vovk.dev/api/static/openapi.json) is exposed via a `GET` endpoint that returns the generated spec as JSON using the `openapi` object imported from `vovk-client/openapi`.
...
The generated specification includes Scalar‑compatible code samples you can copy into your project right away.
...
Running `npm run patch` checks for uncommitted changes, bumps the patch version, and then triggers `postversion`, which generates the clients, bundles the TypeScript client, publishes all three packages, and finally creates a Git commit and tag for the new version.
...
The repository includes CLI demos showing how to invoke RPC methods from Node.js, Rust, and Python. For each language there are two demos:
- `local`: uses the locally generated client.
  - Rust and Python use the `https://hello-world.vovk.dev` origin (to keep the example simple).
  - Node.js uses `http://localhost:PORT` via **vovk-client**; run `npm run dev` in another terminal to start the local server.
- `packaged`: uses the published packages on their respective registries, calling `https://hello-world.vovk.dev`.
  - If Rust and Python envs are installed, `npm run install:all` installs everything; otherwise `npm install` installs Node.js dependencies only.

Each demo calls `updateUser`, `streamTokens`, and `getSpec`.
...
- For Node.js, the only difference is the import path of the client (**vovk-client** instead of **vovk-hello-world**).
- For Python, some workarounds are used to resolve local imports.
- For Rust, the only difference is the dependency path in `Cargo.toml`.

## Conclusion

Although this "Hello World" example implements only three endpoints (`updateUser`, `streamTokens`, and `getSpec`), Vovk.ts provides many features out of the box with minimal configuration, including:

- TypeScript bundling and generation of other language clients that are ready to be published, with detailed READMEs and package metadata.
- A well-documented OpenAPI specification with detailed code snippets powered by Scalar.
- Text streaming and client-side validation across supported languages.

Additional topics not covered here include the [Function Calling](/function-calling) framework (which turns controller/RPC methods into LLM tools), [generating source files only](/templates) (without package files), custom [decorators](/decorators), [imports](/imports) configuration for TypeScript client customization, and more. Explore the documentation to learn more!