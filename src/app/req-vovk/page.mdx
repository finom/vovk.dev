...
While using built-in `NextRequest` functions like `req.json()` and `req.nextUrl.searchParams.get()` are suffecient for most use cases, Vovk.ts also monkey-patches the request object with `vovk` property that provides additional methods for more advanced input data handling. It covers the following scenarios:
