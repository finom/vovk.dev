...
LLM completions can be streamed in the [JSON Lines](https://jsonlines.org/) format—a convenient way to send a sequence of JSON objects over HTTP. On the backend, use [Generator](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Generator) functions to yield data as it becomes available. With the OpenAI API (and other completion APIs), you can delegate the iterable using the `yield*` syntax.

Learn more about the [JSONLines Response](/controller/jsonlines) and [Controllers](/controllers).
...
See the [live example](https://examples.vovk.dev/openai) for details.
...
On the client, use the [@ai-sdk/react](https://www.npmjs.com/package/@ai-sdk/react) package to call the endpoint and build a chat interface following the SDK’s recommended patterns.
...
See the [live example](https://examples.vovk.dev/ai-sdk) for details.
