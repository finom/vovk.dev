...
Allows to redefine imports used by the generated client, such as [fetcher](/customization#fetcher), [validateOnClient](/customization#validateonclient), and [createRPC](/customization#createrpc). The values are paths to the files that export the corresponding functions. Relative paths are resolved from the root of the project, package paths, such as `my-lib/fetcher`, are rendered as is.
...
The `libs` object is a conventional way to define configuration for libraries used by the client or any other configuration that you want to expose to the client. For example, it can be used to define localization for [vovk ajv](/validation#ajv).
