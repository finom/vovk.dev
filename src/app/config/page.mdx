...
The configuration file defines options for the [CLI](/cli), updates [template definitions](/templates), and customizes other settings. In many cases, it’s optional because the CLI can rely on defaults or flags. For more advanced use, creating a config file is recommended.
...
Controls whether to emit config options to [.vovk-schema/_meta.json](/schema). If `true`, emits all options. If set to an array of strings, emits only the listed options. Default: <nobr>`["libs"]`</nobr>.
...
Overrides options for the [bundle](/bundle), including [excludeSegments](/bundle#excludesegments) and [tsdown build options](https://tsdown.ru/reference/config-options).
...
If `"libs"` is present in [emitConfig](#emitconfig), it is emitted to `.vovk-schema/_meta.json` and can be accessed in multiple ways:
...
The `outputConfig` object customizes the generated client code by changing [imports](/imports), `origin`, or adding [OpenAPI Mixins](/codegen).
...
Customizes the imports for `fetcher`, `validateOnClient`, and `createRPC`. See [Imports](/imports) for details.
...
Turns the segment into an OpenAPI Mixin, combining the generated client with third-party APIs. See [OpenAPI Mixins](/codegen) for details.
