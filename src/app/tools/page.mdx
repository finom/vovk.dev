...
- `meta?: Record<string, unknown>{:ts}` - optional metadata passed to each controller/RPC method. The meta can be read on the back end using [req.vovk.meta](/procedure#req-vovk-meta). When passed to a controller procedure, it's merged with procedure-level meta normally. When passed to an RPC method, it's available as `xMetaHeader` key.
...
Annotations can be added to the output by setting a special metadata key `mcpOutput` using [req.vovk.meta](/procedure#req-vovk-meta) function. This approach makes sure that if the procedure used as an endpoint, the response will not be affected.
