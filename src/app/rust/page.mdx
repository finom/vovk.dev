...
In case if you want to generate Rust source only, to be used as part of another Rust project, you can use [rsSrc](/templates#rssrc) template:
...
The [rs](/templates#rs) template as well as some other templates has default `outDir` (by default equals to `./dist_rust`) for the composed client configuration, that can be changed with [template definitions](/templates#defs) in the config:
...
The nested structures can be accessed with the `_::` syntax. For example, in order to get the `body.profile` type from the `update_user_` module, you can use `update_user_::body_::profile` syntax.
