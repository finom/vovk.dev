...
The Rust client can be generated with the `vovk generate` command using the [rs](/templates#rs) template or [rsSrc](/templates#rssrc) template.

You can generate a Rust package with the [CLI](/cli):
...
The package is ready to be published to [crates.io](https://crates.io/) with the `cargo publish` command:

```sh
cargo publish --manifest-path rust_package/Cargo.toml
```

If you want to generate Rust source only to be used as part of another Rust project, you can use the [rsSrc](/templates#rssrc) template:
...
The generation can be [configured](/config) so the client will be generated automatically with the `generate` command with no flags, and also when [vovk dev](/dev) is run, which performs "hot generation" whenever the schema changes. To do that, you need to add the `rs` template to the [composed client](/composed) config:
...
The [rs](/templates#rs) template, as well as some other templates, has a default `outDir` (by default equals to `./dist_rust`) for the composed client configuration, which can be changed with [template definitions](/templates#defs) in the config:
...
...will emit a [Vovk.ts Schema](/schema), which will, in turn, be used to generate the Rust client, following the language conventions where possible, adding comments defined in the schema `description`, and generating the most suitable types. For example, `age` is generated as `u8` (unsigned 8-bit integer) because of the presence of `min` and `max` constraints.
...
All RPC modules are generated in the `lib.rs` file, which contains all the RPC methods as functions as well as types that are used in the RPC methods. Nested data structures are generated as nested modules containing corresponding `struct` definitions or other types.

The nested structures can be accessed with the `_::` syntax. For example, to get the `body.profile` type from the `update_user_` module, you can use the `update_user_::body_::profile` syntax.

The structs can be imported and renamed with a `use` statement to follow PascalCase naming convention:
...
Behind the scenes, it uses [reqwest](https://docs.rs/reqwest/latest/reqwest/) for HTTP requests, [jsonschema](https://docs.rs/jsonschema/latest/jsonschema/) for client-side validation, and some other common libraries.

### JSON Lines Endpoints

To implement continuous streaming with [JSON Lines](/controller/jsonlines) endpoints, the client uses the `Iterator` trait to return an async iterator that can be used to iterate over the streamed data.
...
pub fn main() {
  let stream_response = stream_rpc::stream_tokens(
      (), // () as no body, query, or params are expected
      (),
      (),
      None,
      None,
...
- 🧩 Use `.await?{:rs}` instead of blocking calls.
