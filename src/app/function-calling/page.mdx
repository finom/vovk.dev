...
Controllers as well as generated RPC modules can be converted into AI tools for LLM function calling with minimal effort, using `deriveTools` utility. This makes your back-end functionality accessible to AI models (including MCP clients) with almost no extra code.

```ts showLineNumbers copy filename="src/modules/user/UserController.ts" {4-12}
...
- `modules: Record<string, object>` - a record of modules (RPC modules or controllers) to derive tools from.
- `onExecute?: (tool: VovkTool, result: KnownAny) => void` - optional callback invoked when a tool's `execute` function completes successfully.
...
This feature is implemented by a zero-dependency function, `deriveTools`, imported from `"vovk"`. It produces a `tools` array that can be mapped to function-calling inputs for any LLM library. Each tool, converted from a controller or RPC handler, implements the `VovkTool` interface with the following properties:
...
- `description`: a description derived from OpenAPI metadata, concatenating the `summary` and `description` fields, or overridden by a custom `x-tool-description`. If neither is provided, `deriveTools` ignores the function. In other words, using the `@operation` decorator on [controller](/controller) methods is required for this feature.
...
`deriveTools` accepts an options object with:
...
Because the Vovk.ts CLI can serve as a standalone code generator (e.g., for [NestJS](/nestjs)), you can still use the generated RPC modules with `deriveTools`.

## Function Calling Example

You can implement function calling using raw AI APIs with [JSON Lines](/jsonlines). In most cases, including this example, the [Vercel AI SDK](https://ai-sdk.dev/) provides a simple way to build a text chat with function calling.

### Create LLM Endpoint

Create a controller and pass your RPC modules or controllers to `deriveTools` as shown above.
...
- ✨ Add a `router` option to `deriveTools` to support hundreds of functions without hitting LLM function-calling limits. Routing can be implemented using vector search or other approaches.
