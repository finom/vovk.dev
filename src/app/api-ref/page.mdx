...
A utility function that turns RPC modules or [controllers](/controller) into LLM tools based on the input schemas of their methods.
...
Key-value object where keys are module names and values are objects with methods. Each method should have a `schema: VovkHandlerSchema` property and either `isRPC: true` (for RPC modules) or the [fn](/controller/fn) function (for controllers).
...
See the [request](/controller/req-vovk) documentation for more info.
...
Function that creates RPC module out of the [schema](/schema) and types inferred from [controllers](/controller). It's imported by the generated TypeScript, ESM, and CJS files and can be [customized in the config](/imports).
