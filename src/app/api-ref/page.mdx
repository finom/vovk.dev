...
Decorator application follows the same order as the stacked syntax: the last decorator in the list (closest to the handler) is applied first. See [Decorators Overview](/decorator-overview) for guidance on choosing between `decorate` and decorator syntax.
...
- `onExecute?: (result: unknown, tool: StandardToolV0) => void | Promise<void>{:ts}` – called after each tool execution.
- `onError?: (e: Error, tool: StandardToolV0) => void | Promise<void>{:ts}` – called when an error is thrown during tool execution.
...
Read more in the [Progressive Response](/progressive) documentation.
...
See the [req.vovk](/req-vovk) documentation for more information.
