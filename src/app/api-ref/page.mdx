...
A utility function that turns [RPC modules](/rpc-module) or [controllers](/controller) into LLM tools based on the input schemas of their methods.
...
Extends `OperationObject` imported from the [openapi3-ts/oas31](https://www.npmjs.com/package/openapi3-ts) package by adding `x-tool` property, typed as `VovkToolOptions`.
...
Function that creates [RPC module](/rpc-module) out of the [schema](/schema) and types inferred from [controllers](/controller). It's imported by the generated TypeScript, ESM, and CJS files and can be [customized in the config](/imports).
