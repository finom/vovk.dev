export const metadata = {
  ...
  description:
    'After testing in multiple large projects, thanks to community feedback, Vovk 2.0 is released with some insignificant breaking changes.',
  date: '11/05/2024',
};
...
`onError` option of `initSegment` now gets request object as the second argument. This is useful if you want to extract some information about request like headers, method, authorisation, etc.
...
const { GET, POST, PUT, DELETE } = initSegment({
  controllers,
  workers,
