...
<Bleed>
<div className="sticky-comparison">
...
| [**Deepkit**](#deepkit) | ✅ Native (HTTP) | ❌ Own runtime | 🔌 runtime types | ✅ Pure inference (RPC) | ❌ | ✅ `DirectClient` (RPC) |
...
- **Multi-language client codegen.** First-party [`vovk-python`](https://github.com/finom/vovk-python) and [`vovk-rust`](https://github.com/finom/vovk-rust) emit typed clients in Python and Rust from the same `.vovk-schema`. 🧪 experimental. ([Encore.ts](#encorets) also emits non-TS clients first-party — Go — via `encore gen client`; Vovk is the one on this list emitting Python and Rust.)
...
**Versions**: `@igniter-js/core@0.3.40`, `@igniter-js/adapter-mcp-server@0.3.7`. **Pre-1.0** (a `1.0.0-alpha` line exists on npm but no GA). Development has been quiet — no published release since December 2025 — and the ecosystem is thin and largely single-maintainer. Source: [igniterjs.com](https://igniterjs.com).
...
Per the package README, the adapter exposes your entire Igniter.js API as MCP tools so agents can interact with your endpoints as native functions — no separate definitions required. *Source: [github.com/felipebarcelospro/igniter-js — adapter-mcp-server README](https://github.com/felipebarcelospro/igniter-js/tree/main/packages/adapter-mcp-server).*
...
const result = await greeting.ping({ name: 'world' });
```
...
**Version**: `@tanstack/react-start@1.167.65`. (Uses the unified TanStack monorepo version — there is no separate "1.0 GA" milestone; non-prerelease builds have shipped since early 2025.) Source: [tanstack.com/start](https://tanstack.com/start).
...
**Versions**: `hono@4.12.18`, `@hono/mcp@0.3.0`. Source: [hono.dev](https://hono.dev).
...
**Versions**: `@deepkit/http@1.0.19`, `@deepkit/rpc@1.0.19`, `@deepkit/type@1.0.19`. Source: [deepkit.io](https://deepkit.io).
...
*Source: [deepkit.io/documentation/http](https://deepkit.io/documentation/http)*
...
*Source: [deepkit.io/documentation/runtime-types](https://deepkit.io/documentation/runtime-types)*
...
*Source: [deepkit.io/documentation/rpc](https://deepkit.io/documentation/rpc)*
...
*Source: [deepkit.io/documentation/package/rpc](https://deepkit.io/documentation/package/rpc)*
...
- 'auto-webmcp': 0.3.24
- '@modelcontextprotocol/sdk': 1.29.0 (latest; NO v2 / pre-release line published)
- '@modelcontextprotocol/hono': 2.0.0-alpha.2 (adapter pkg, separate from sdk)
- '@modelcontextprotocol/express': 2.0.0-alpha.2
- '@modelcontextprotocol/node': 2.0.0-alpha.2
- '@modelcontextprotocol/fastify': 2.0.0-alpha.2 (dedicated Fastify adapter — exists)

WebMCP spec movement (re-verified 2026-05-29 against W3C spec + repo PRs + developer.chrome.com):
- WebMCP is W3C Draft Community Group Report (WebML CG), first published Aug 2025, living doc. NOT a Standard.
- Chrome 146 reached stable 2026-03-10, but WebMCP is Canary-ONLY (behind a flag); NOT on stable. Origin trial expected ~Chrome 149.
- 2026-03-05: provideContext()/clearContext() removed from spec (PR #132)
- late March 2026: unregisterTool() dropped for AbortSignal-driven unregistration (PRs #147/#156, ~Mar 26-27) — NOT 2026-04-23
- 2026-05-27: getter moved Navigator -> Document; now document.modelContext (navigator.* = deprecated alias) (PR #184)
- orpc-webmcp is an UNPUBLISHED demo (Mark-Life/webMCP-example), NOT an npm package
- webmcp npm pkgs jumped to 3.0.0 on 2026-05-29 (were 2.3.2 at snapshot)
