...
description: "A six-axis comparison of ~30 TypeScript backend frameworks — REST URL semantics, Next.js compatibility, validation library, typed RPC client, LLM tool derivation, and validated local call. Verified against npm and official docs."
date: "May 11, 2026"
---
...
All version numbers and code examples were verified against official documentation and npm on **2026-05-11**, and re-verified on **2026-05-25** (no structural claims changed; affected version bumps reflected in the metadata block at the end).
...
A note on the LLM-tool column: none of the four 4/4 SSOT frameworks (Vovk, Nestia, oRPC, Igniter.js) ships an end-to-end MCP server transport. Vovk's `deriveTools()` produces tool objects with a `ToModelOutput.MCP` formatter; the transport is delegated to `mcp-handler`. oRPC's `@orpc/ai-sdk` produces ai-sdk tool definitions; MCP server requires `@modelcontextprotocol/sdk` directly. Nestia's `@agentica` runtime is comprehensive but again does not bundle an MCP HTTP transport. **Only Igniter.js packages its own MCP server end-to-end via `@igniter-js/adapter-mcp-server`** — strictly more first-party than the others on this sub-feature.

---

## Beyond the six axes: where the Tier S frameworks differentiate

The six axes were chosen for a tight, defensible comparison. But several secondary capabilities meaningfully differentiate the four frameworks that hit 4/4 SSOT. This table tries to be honest about where each leads and where each lags:
...
The rubric stays abstract by design; this section commits to recommendations. If your project matches one of these descriptions, that's the framework to start with.
...
This isn't exhaustive — the per-framework breakdowns below cover the trade-offs each pick implies. But if you came looking for one-line guidance, that's the matrix.
...
**Verified version**: `vovk@3.2.2` (npm 2026-05-11). Requires Next.js 15+, Node 22+. Source: [vovk.dev](https://vovk.dev).
...
Streaming (JSON Lines via `iteration` validator) is a first-class procedure-level capability, but oRPC has parity via its [Event Iterator](https://orpc.dev/docs/event-iterator) (SSE) — so it's listed in the bonus axes table above rather than claimed as a Vovk-specific lead.
...
**Verified version**: `@nestia/core@11.2.0`. Built on NestJS 11. Requires `typia` compile-time transformer. Source: [nestia.io](https://nestia.io).
...
**Verified versions**: `@orpc/server@1.14.2`, `@orpc/client@1.14.2`, `@orpc/openapi@1.14.2`, `@orpc/ai-sdk@1.14.2` (npm 2026-05-11). Runtime-agnostic. Source: [orpc.dev](https://orpc.dev).
...
**Verified versions**: `@igniter-js/core@0.3.40`, `@igniter-js/adapter-mcp-server@0.3.7` (npm 2026-05-11). **Pre-1.0** — moving fast, ecosystem is thin. Source: [igniterjs.com](https://igniterjs.com).
...
**Verified versions**: `@effect/platform@0.96.1`, `@effect/ai@0.35.0`. Source: [effect.website](https://effect.website).
...
**Verified version**: `encore.dev@1.57.1`. Source: [encore.dev/docs/ts](https://encore.dev/docs/ts).
...
**Verified version**: `elysia@1.4.28`. **Optimized for Bun, also runs on Node.js.** Source: [elysiajs.com](https://elysiajs.com).
...
**Verified version**: `@tanstack/react-start@1.167.65` (v1 stable since March 2026). Source: [tanstack.com/start](https://tanstack.com/start).
...
**Verified versions**: `@trpc/server@11.17.0`, `trpc-to-openapi@3.2.0` (community fork). Official `@trpc/openapi@11.17-alpha` exists. Source: [trpc.io](https://trpc.io).
...
**Verified version**: `@ts-rest/core@3.52.1`. Source: [ts-rest.com](https://ts-rest.com).
...
**Verified versions**: `hono@4.12.18`, `@hono/mcp@0.2.5`. Source: [hono.dev](https://hono.dev).
...
**Verified version**: `@nestjs/core@11.1.19` (vanilla NestJS, without Nestia). Source: [docs.nestjs.com](https://docs.nestjs.com).
...
**Verified version**: `fastify@5.8.5`. Source: [fastify.dev](https://fastify.dev).
...
**Verified version**: `next-safe-action@8.5.2`. Source: [next-safe-action.dev](https://next-safe-action.dev).
...
**Verified version**: `zsa@0.6.0`. Source: zsa.vercel.app.
...
**Verified version**: `mcp-handler@1.1.0`. Source: [github.com/vercel/mcp-adapter](https://github.com/vercel/mcp-adapter).
...
This comparison is authored by the maintainer of [Vovk.ts](https://vovk.dev). The six-axis rubric was fixed before per-framework analysis began, and every claim about every other framework is sourced inline against that framework's official docs or npm. If you find a Vovk-vs-competitor claim that doesn't hold against the linked source, the article is wrong — please open an issue.

All data verified against:

- **npm registry** — versions snapshotted on 2026-05-11, re-verified on 2026-05-25
- **Official documentation pages** — linked inline per framework
- **Package READMEs** on GitHub where docs were unavailable

Code examples are quoted verbatim from each framework's docs or transposed minimally for consistency across the article. We did not reproduce examples we couldn't verify against current docs — if a section is missing in our notes, we said so rather than fabricate.
