...
**[Local procedure execution](/fn).** Every procedure exposes a `.fn()` method that runs in the current context — no HTTP round-trip. This unlocks SSR, PPR, server actions, background jobs, and AI agent execution, all using the same handler logic.
