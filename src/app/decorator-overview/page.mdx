...
- `headers?: Record<string, string>` — custom response headers. See [Response Headers](/response#headers) for dynamic headers.
- `staticParams?: Record<string, string>[]` — (`@get` only) static params for `generateStaticParams()`. See [Static Segment](/static-segment) for details.
