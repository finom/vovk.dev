...
Every RPC method has a set of options that can be passed to it, besides `body`, `params` and `query`. The list of options can be [customized with a custom `fetcher`](/imports#fetcher).
...
If iterable is returned, the client library is going to cast the method as a disposable async generator to implement [JSONLines](/controller/jsonlines) response streaming.
...
The generated client library also includes `openapi` and `schema` exports that can be used to access the OpenAPI spec and the JSON Schema respectively.
...
This also works at [segmented client](/segmented):
