...
`updateUser` performs [client-side validation](/validation/client), serializes `query` and `params` into the URL, and issues a standard `fetch` request. The server handles it in `UserController.updateUser`. The RPC method returns a promise that resolves to the return type used in the controller method.
