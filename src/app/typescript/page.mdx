...
The client library's fetching function, as well as its types can be customised in order to follow logic required by the application. See [`fetcher` customization docs](/imports#fetcher) for more details.
...
If iterable is returned, the client library is going to cast the method as a [disposable](https://github.com/tc39/proposal-explicit-resource-management) [async iterators](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/AsyncIterator) to implement [JSONLines](/controller/jsonlines) response streaming.
...
- `fullSchema` - the full schema object of type `VovkSchema` that includes all available segments but also emitted config (by default `"libs"` and `"generatorConfig"` options only, see [config documentation](/config)).
...
This also works at [segmented client](/segmented), where both `openapi` and `schema` contain data only for the used segment:
...
- [cjs](/templates#cjs), [mjs](/templates#mjs) (`requires` [schemaCjs](/templates#schemacjs) and [openapiCjs](/templates#openapicjs)) - compiled CJS and ESM modules with typescript definitions; used by default at [composed client](/composed);
- [ts](/templates#ts) (`requires` [schemaTs](/templates#schemats) and [openapiTs](/templates#openapits)) - uncompiled TypeScript module with typescript definitions; used by default at [segmented client](/segmented);
- [mixins](/templates#mixins) - `.d.ts` types and `.json` file that are generated when [OpenAPI Mixins](/codegen) are used;
- [readme](/templates#readme), [packageJson](/templates#packagejson) - `README.md` that contains RPC documentation and `package.json` that can be used to publish the generated library as a NPM package.
