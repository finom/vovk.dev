...
3. On change, the script checks whether the file contains a controller and belongs to a [segment](/segment) using simple RegExp checks.
...
5. If the schema differs:
   ...
   - If the controller list changed, the client is also regenerated. The client imports schema JSON files to initialize the exported library. By default, the [composed client](/composed) is generated in `./src/client` (or `./client` without a `src` folder). When the [segmented client](/segmented) is enabled, per-segment folders are generated in the same directory.
