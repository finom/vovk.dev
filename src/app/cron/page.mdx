...
[Vercel Cron jobs](https://vercel.com/docs/cron-jobs) require simple authorization using an environment variable. This can be achieved by creating a [decorator](/controller/decorator) that checks authorization header with a secret key.
...
Add the cron job to the `vercel.json` file. The `schedule` field uses the cron syntax (at this case the cron job will run every day at midnight).
