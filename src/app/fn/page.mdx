...
Every procedure created with the [procedure](/procedure#procedure) function is callable as a regular function via its `fn` property. The call signature is `{ params, query, body }` — the same shape as the generated RPC client, which mirrors `fn` (not the other way around).
...
When the controller is mounted in a [segment](/segment), the same procedure is also reachable over HTTP through the generated RPC client — same call shape:
