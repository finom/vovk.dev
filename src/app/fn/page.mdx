...
Another thing to keep in mind is that the local procedure is [decorated](./decorator) with the same decorators as the HTTP handler. If you create a decorator to wrap an HTTP handler for use with `fn`, avoid using `NextRequest`‑specific properties like `req.url`, `req.headers`, or `req.nextUrl`.
...
You can detect the local procedure context by checking `req.url`, which is `undefined` when called via `fn`:
...
const myDecorator = createDecorator((req, next) => {
  if (typeof req.url === 'undefined') {
    console.log('Local handler context');
  } else {
