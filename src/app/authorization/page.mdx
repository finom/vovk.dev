...
Authorization of requests can be implemented using [decorators](/controller/decorator) that checks if a user is authorised and passes down useful information such as current user to the controller handler via [metadata](/controller/meta) that can be accessed via `req.vovk.meta()` method.
...
Import the `authGuard` decorator and related members and apply it to the controller methods, passing it after the `@get`, `@post`, etc. decorators.
