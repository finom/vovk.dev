...
The plugin lives in the same monorepo as Vovk.ts itself, at <a href="https://github.com/finom/vovk/tree/main/packages/claude-plugin">packages/claude-plugin</a>, and ships alongside the framework on every release.
...
For a local checkout (development), substitute the path: `claude plugin marketplace add /path/to/vovk` (point at the repo root, not at `packages/claude-plugin/`).

### Verify

In Claude Code, run `/plugin` — the **Installed** tab should list `vovk`. Skills are namespaced; typing `/vovk:` (with the trailing colon) lists all 14 skills available to the agent. In other agents, the skill files appear under that agent's skill directory (e.g. `.cursor/skills/`).
