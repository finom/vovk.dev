...
- **Source-verified.** Every technical claim is pinned to a file path and line number in `packages/vovk/`. Skills are regenerated against source on every release, so they don't lag the framework.
...
- **Token-efficient prose.** Skill markdown uses a terse fragment style — articles, filler, and hedging dropped while every code block, file path, line-number pin, and cross-reference is preserved verbatim. Roughly 10% fewer tokens per skill load with no loss of substance.

The framework underneath is built for this kind of pairing: plain REST with end-to-end types, [OpenAPI native](/openapi), [multitenancy baked in](/multitenant), and procedures that double as [AI tools](/tools) via `deriveTools`. Logic groups under `src/modules/<name>/` (Controller plus Service per module — not scattered across `lib/`), and the same `procedure().handle()` powers the HTTP endpoint, the [SSR call](/fn), and the AI tool — one [single source of truth](/inference). Fewer files, predictable layout, less for the model to reconcile.
