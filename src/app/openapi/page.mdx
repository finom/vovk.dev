...
You can use the specification directly as a variable or expose it via a static segment with a simple [controller](/controller) that serves it as a JSON endpoint.
...
The `@operation` decorator also provides `tool` property that defines tool-specific attributes for [deriveTools](/function-calling) function. It's set under `x-tool` key in the OpenAPI operation object.
...
For more details, see the [deriveTools](/function-calling) documentation.