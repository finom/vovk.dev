...
Input and output inference is performed by universal types that can be applied to both RPC modules and [controller](/controller) methods.
...
The source body, query, and params types are defined using [`VovkRequest<TBody, TQuery, ?TParams>{:ts}`](/controller/request), which defines the type of the `req` argument of the controller method. In other words, both raw method definitions and validated method definitions define the input types for `body`, `query`, and `params`.

Raw method definition with param as a generic argument:
...
Raw method definition with param as a separate argument:
...
Validated methods define the input types automatically:
...
All three cases will generate the same RPC method:
...
And all these cases will allow you to infer the input types in the same way:
...
The `VovkReturnType<T>` and `VovkYieldType<T>` types allow you to infer the actual return type or yield type of a method when its input isn't validated. These types cannot be used for self-references in [services](/controller/service), as they cause "implicit any" TypeScript errors.
