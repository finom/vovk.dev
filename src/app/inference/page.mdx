...
Validated [procedures](/procedure) infer input types automatically:
...
Output and iteration types are set only when using [procedure](/procedure) function.
...
For [JSON Lines](/jsonlines) responses:
...
> [!IMPORTANT]
> 
> Procedure return type and output type are independent to avoid self-inference issues. For stricter type safety, use `satisfies VovkOutput<T>{:ts}` or `satisfies VovkIteration<T>{:ts}` to refer to the output/iteration types.
> ```ts showLineNumbers copy
> import { get, type VovkOutput } from 'vovk';
> import { z } from 'zod';
> 
> export default class UserController {
>  @get()
...
>    output: z.object({ success: z.boolean() }),
>    async handle(req, params) {
>      return { success: true } satisfies VovkOutput<typeof UserController.updateUser>;
>    },
>  });
>}
>```
