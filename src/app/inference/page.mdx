...
Source types for `body`, `query`, and `params` are defined via `VovkRequest<TBody, TQuery, ?TParams>{:ts}`, which specifies the type of the `req` argument in controller methods. In other words, both raw and validated method definitions determine the input types.
