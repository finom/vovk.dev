...
And all these cases will allow to infer the input types the same way:
...
For [JSONLines](/controller/jsonlines) responses:
...
The `VovkReturnType<T>` and `VovkYieldType<T>` types allow to infer the actual return type or the yield type of a method when it's input isn't validated. The types cannot be used for self-references at [services](/controller/service) as they cause "implicit any" TS errors.
