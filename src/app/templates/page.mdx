...
Vovk.ts renders the client libraries from [EJS](https://www.npmjs.com/package/ejs) templates. To outpower potential use cases that need to be met, Vovk.ts provides simple yet powerful template logic that satisfies the following requirements:

1. Render multiple files from a single template definition. That means that a **"template" is a directory** with one or more files, where files with extenshion `.ejs` are rendered as EJS templates, and other files are copied as is. `index.ts.ejs` file is rendered as `index.ts` file, and `index.ts` without `.ejs` extension is copied as is.
2. Use existing templates as a base for other templates using `extends` option. That means that you can create a new template that extends an existing one and overrides some of its options.
3. Use existing templates as dependencies for other templates using `requires` option. That means that you can create a new template that uses existing templates as dependencies and renders them in the output directory.

Template definitions are configured in the [config](/config) file under `clientTemplateDefs` option. By default the value of this object includes built-in template definitions that can be viewed if you set [`emitConfig`](/config#emitconfig) option to `true` or, if `emitConfig` is an array, add `clientTemplateDefs` to the list. After that you can take a look at the `.vovk-schema/_meta.json` file that contains the list of all available template definitions under the `config` key.

## Example

To make it easier to understand the options in the section below, let's break down a built-in template definition for the [Rust client](/rust). The primary template definition is stored under the [rs](#rs) key, indicating the template name that can be used as `fromTemplates` item in the [segmented client](/segmented) or [composed client](/composed) configuration. 


By itself it doesn't have files to render, so it doesn't include `templatePath` option. Instead, it extends [rsSrc](#rssrc) template definition that renders the Rust source code files only to `./src` dir related to `outDir`, [rsPkg](#rspkg) template definition that renders Cargo-related files to the root of `outDir` and [rsReadme](#rsreadme) template definition that renders README files to the root of `outDir`. The `rs` template definition also defines `composedClient` option that overrides the default `outDir` option to `dist_rust`, so it isn't compiled into `node_modules/.vovk-client` folder used by the composed [TypeScript](/typescript) client.
...
(all paths are ended with a slash `/` for clarity, but it isn't required)
...
Overrides the `generatorConfig` option in the root of [config](/config#generatorconfig) file. This can be used to customize the generated client for this template to use different `origin`, `openAPIObject`, etc.
...
- `requires` [schemaTs](#schemats), [openapiTs](#openapits) and [mixins](#mixins) (the last one is used conditionally if OpenAPI mixins are used).
...
Renders `schema.ts` file that imports the schema of available segments from `.vovk-schema/` folder and re-exports it as a TypeScript object.
...
Renders `openapi.ts` file that re-exports the OpenAPI schema located at `openapi.json` file (provided by [openapiJson](#openapijson) template) at the same folder.
...
Renders `openapi.cjs` and `openapi.d.cts` files that re-export the OpenAPI schema located at `openapi.json` file (provided by [openapiJson](#openapijson) template) at the same folder.
...
- 📝 Document custom client templates and available variables.