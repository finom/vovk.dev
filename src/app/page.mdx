...
Procedures can run directly on the server for SSR/PPR and server actions, skipping the HTTP round-trip. See [Calling Procedures Locally](/fn).
...
The official **Claude Code plugin** ships 14 topic-based skills that teach the coding agent how to use Vovk.ts when you describe what you want to build. Skills load only when relevant — typing *"scaffold a tenant"* loads the multitenant skill, *"stream chat tokens"* loads JSON Lines.
