...
Define endpoints once (as real Next.js handlers), and Vovk.ts **emits schema artifacts** to **generate type-safe clients**, **publish docs**, **build AI tools**, and **mix in third-party OpenAPI schemas**—without writing files for contract layer.
...
- Don’t use Next.js
- Need runtime access to original schema (JSON Schema is available, though; contracts can still be moved to a separate file if you want)

## See it in the Hello World example
...
- Generated OpenAPI rendered via Scalar
...
## Segments: split your back-end into independent units
...
## Standard Schema–compatible validation (Zod, Valibot, ArkType)
...
- **Your handlers are the source of truth**.
...
## Runtime and toolchain separation
