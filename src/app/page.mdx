...
<div className="flex justify-center gap-2 mt-6 mb-8">
  ...
  <a href="/context/docs.md">
    <img alt="Docs Context" src="https://img.shields.io/badge/ai_context-docs.md-white" />
  </a>
  <a href="/context/realtime-ui.md">
    <img alt="Realtime UI Context" src="https://img.shields.io/badge/ai_context-realtime_ui.md-white" />
  </a>
</div>
...
Vovk.ts adds a structured API layer on top of Next.js App Router Route Handlers. Define a procedure once — same code runs locally for SSR, mounts as an HTTP endpoint when you add a decorator, and exposes as an LLM tool via `deriveTools`. The framework emits schema artifacts that generate type-safe clients, OpenAPI docs, and AI tool definitions. No separate contract layer to maintain.
...
> Requires Node.js 22+ and Next.js 15+. &nbsp; [Quick Start](/quick-install) · [Manual Install](/manual-install) · [Claude Plugin](/claude) · [GitHub](https://github.com/finom/vovk)

---

## What it looks like

A procedure is a typed, validated callable. Define inputs and output with [`procedure`](/procedure) — params, query, body — and call it directly on the server for SSR, server components, or server actions:
...
Procedures can yield JSON Lines for real-time responses. See [JSON Lines](/jsonlines).

### Local procedure calls

Procedures call directly on the server with `.fn()` — same call shape as the generated HTTP client. Use them in SSR/PPR, server components, and server actions. See [Calling Procedures Locally](/fn).
...
|---------|------|--------|---------|
| **`vovk`** | Runtime — decorators, `procedure`, routing, `deriveTools` | [![NPM Version](https://img.shields.io/npm/v/vovk)](https://www.npmjs.com/package/vovk) | production |
| **`vovk-cli`** | Toolchain — codegen, mixins, docs, bundling | [![NPM Version](https://img.shields.io/npm/v/vovk-cli)](https://www.npmjs.com/package/vovk-cli) | dev |
| **`vovk-client`** | Re-exported composed client | [![NPM Version](https://img.shields.io/npm/v/vovk-client)](https://www.npmjs.com/package/vovk-client) | production (optional) |
| **`vovk-ajv`** | Client-side validation with AJV | [![NPM Version](https://img.shields.io/npm/v/vovk-ajv)](https://www.npmjs.com/package/vovk-ajv) | production (optional) |
| **`vovk-python`** | Python client generation (experimental) | [![NPM Version](https://img.shields.io/npm/v/vovk-python)](https://www.npmjs.com/package/vovk-python) | dev (optional) |
| **`vovk-rust`** | Rust client generation (experimental) | [![NPM Version](https://img.shields.io/npm/v/vovk-rust)](https://www.npmjs.com/package/vovk-rust) | dev (optional) |
...
See [Claude Plugin](/claude) for the full skill list, the AI mind model behind the framework, and why the pairing works.
...
The [Multitenancy Tutorial](/multitenant) walks through hosting multiple tenants from different subdomains within a single Next.js app.

The [Realtime UI](/realtime-ui) tutorial series builds a full-stack Kanban board where users, bots, AI agents, and MCP clients update the board in real time — covering state normalization, database polling, AI chat, voice AI, and Telegram integration.
...
| **Procedure** | A typed, validated callable created with [`procedure`](/procedure). Call it locally with `.fn()`, expose it over HTTP with a decorator, or derive it as an LLM tool |
