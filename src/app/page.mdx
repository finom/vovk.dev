...
Every group of controllers lives in a [segment](/segment) — a Next.js catch-all route that compiles into its own serverless function. Segments are configured independently: different middleware, different auth, different deployment characteristics.
...
Direct type mapping between server and client code gives you jump-to-definition and JSDoc on hover over generated RPC methods. See [Schema](/schema).
...
See [Calling Procedures Locally](/fn).
