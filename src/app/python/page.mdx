...
The Python client can be generated with `vovk generate` using the [py](/templates#py) or [pySrc](/templates#pysrc) template.

Create a Python package with the [CLI](/generate) command:
...
For continuous streaming with [JSON Lines](/jsonlines) endpoints, the Python client returns a `Generator` you can iterate over.
...
- 🐞 Allow circular $refs with [OpenAPI mixins](/mixins).
