...
The [py](/templates#py) template as well as some other templates has default `outDir` (by default equals to `./dist_python`) for the composed client configuration, that can be changed with [template definitions](/templates#defs) in the config:
...
... will emit [Vovk.ts schema](/schema), that in its turn will be used to generate the Python client, following the language conventions where possible, adding comments defined at schema `description`, and generating most suitable number types. For example, the `age` is generated as `int` as per the controller method definition.
...
All RPC modules are generated in the `__init__.py` file, that contains all the RPC methods as functions but also types that are used in the RPC methods. Types for nested data structures are generated as `TypedDict`s.
...
To implement continious streaming with [JSONLines](/controller/jsonlines) endpoints, Python client implements `Generator` that can be used to iterate over the streamed data.
...
- ✨ Generate importable types for named schemas defined at `components/schemas`.