...
This guide will walk you through the steps to implement multitenancy in your Next.js application with a little help from Vovk.ts, making it possible to serve different areas of your application under different subdomains, illustrated by implementing the following areas, showcasing multiple use cases:
...
Each tenant has its own root API endpoint, served from the corresponding domain's `/api` path. In other words, the API for the customer tenant is served from `customer.example.com/api`, and the API for the admin tenant is served from `admin.example.com/api`. These root endpoints are implemented as [segments](/segments) and handled by the Next.js middleware to rewrite the request to the appropriate path based on the tenant subdomain.

For the front-end, you'll use the built-in Next.js [dynamic routes](https://nextjs.org/docs/app/api-reference/file-conventions/dynamic-routes) to handle tenant-specific paths. For example, the specific customer tenant page will have its own page file `src/customer/[customer_name]/page.tsx`, and the admin tenant will use `src/admin/page.tsx`.

Vovk.ts provides a tiny routing utility called `multitenant` that accepts request information and returns the action to take, such as redirecting to a specific subdomain or rewriting the request to a different path. It will be used in the Next.js middleware to handle the request and redirect or rewrite it to the appropriate path based on the tenant subdomain.

<Steps>
## Configure DNS

To set up [multitenant.vovk.dev](https://multitenant.vovk.dev/) with the subdomains, I've used Vercel as a hosting provider. The DNS records are configured as follows:
...
See the [Vercel documentation](https://vercel.com/docs/domains/working-with-domains/add-a-domain) for more information on configuring domains or check your hosting provider's documentation for configuring wildcard subdomains.

In the screenshot, the following domains are configured:
...
After [setting up a Vovk.ts](/getting-started) application, the easiest way to get started is to use the Vovk.ts CLI to create the segments and controllers. The CLI will generate the necessary files for you, so you can focus on implementing the business logic.
...
Next, create the controllers for each segment. For example, to create `ProductService` and `ProductController` for the root segment in the `src/modules/product/` folder, use:

```sh npm2yarn
npx vovk new controller service product
```

Create the `UserService` and `UserController` for the admin segment in the `src/modules/admin/user/` folder:
...
By default, Vovk.ts uses a so-called "composed client" emitted to the `node_modules/.vovk-client` folder, which is, in turn, re-exported from the `"vovk-client"` package. The modules import all [schemas](/schema) from the `.vovk-schema` folder, making the entire app schema visible for inspection in every front-end module that imports the client.

This problem is solved by introducing a [segmented client](/segmented) that generates a separate client for each segment, allowing you to import only the schemas that are relevant to the used segment. Each segment will have its own directory in the project folder, and the client will import only the schema that is relevant to that segment.

This approach also allows higher-order segments (such as "admin") to import lower-order segments (such as "customer"), making it possible to share the RPC libraries between segments while keeping the back-end implementation invisible to pages where its RPC modules are not used.
...
To do that, you need to disable the composed client (in this case, you don't need the `"vovk-client"` package) and enable the segmented client in your [config file](/config).
...
The `"vovk"` package provides a utility function called `multitenant` that plays the role of a router for your multitenant application. It accepts the request information and returns the action that `middleware` needs to take, such as redirecting to a specific subdomain or rewriting the request to a different path.
...
- `overrides`: an object that maps tenant subdomain names to their specific routing rules. Each rule is an array of objects with `from` and `to` properties, where `from` is the path prefix to match and `to` is the path to which the request should be rewritten or redirected.
