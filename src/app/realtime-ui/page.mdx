...
In order to demonstrate this feature, I've created a demo project of a kanban board that you can find at [GitHub repository](https://github.com/finom/vovk-ai-demo). The database, implemented with Postgres and Prisma ORM, is going to have only two tables: users and tasks, while it can be easily extended to include more tables in a real application. Note that the described project is just a demo and might need additional optimizations and improvements to be used in production.
...
The authentication is made by oversimplifying a solution provided at the official [Next.js authentication documentation](https://nextjs.org/docs/app/guides/authentication). It implements a [`/login` page](https://github.com/finom/vovk-ai-demo/blob/main/src/app/login/page.tsx) with a form that invokes [`login` server action](https://github.com/finom/vovk-ai-demo/blob/main/src/app/actions/auth.ts). Session is created at [src/lib/session.ts](https://github.com/finom/vovk-ai-demo/blob/main/src/lib/session.ts) and Data Access Level file is defined at [src/lib/dal.ts](https://github.com/finom/vovk-ai-demo/blob/main/src/lib/dal.ts).

The DAL file, in its turn, exports `verifySession` function that is invoked at [page.tsx](https://github.com/finom/vovk-ai-demo/blob/main/src/app/page.tsx) and redirects the user to the login page if the session is invalid.
...
<GithubFiles paths={['src/decorators/sessionGuard.ts']} repo="vovk-ai-demo" />
...
<GithubFiles paths={['prisma/schema.prisma']} repo="vovk-ai-demo" />
...
For more details, check the full code of the component at the [GitHub repository](https://github.com/finom/vovk-ai-demo/blob/main/src/components/RealTimeDemo.tsx) as well as the [Floaty component](https://github.com/finom/vovk-ai-demo/blob/main/src/components/Floaty.tsx).

<a id="deploy" />
## Deploy the app

As this is just a demo, I've used free proprietary tools and services to make it work for easy deployment. Vovk.ts isn't affiliated with any of them, and you can replace them with any other tools you prefer (switching out from Neon would require minor changes to [DatabaseService.ts](https://github.com/finom/vovk-ai-demo/blob/main/src/modules/database/DatabaseService.ts) to remove Neon-specific code).
...
Add `OPENAI_API_KEY` at the project environment variables. Other variables such as `DATABASE_URL` and `REDIS_URL` are going to be created automatically by the integrations. In case of a problem, check the [.env.template](https://github.com/finom/vovk-ai-demo/blob/main/.env.template) file for reference.
