...
[State normalization](./state), a custom [fetcher](./fetcher) and corresponding back-end implementation makes UI updated when the user interacts with the app by making HTTP requests. The implementation also covers AI features, such as [Text Chat AI](./text-ai) and [Voice AI](/realtime-ui/voice-ai), explained in the further articles. The rule of thumb is that the data always needs to be processed by the entity registry.

But what if the database is changed by other users or third-party services to keep the UI in sync with the back-end data in real-time? One of the ways to achieve this is by implementing database polling powered by [JSONLines](/jsonlines). The server sends updates to clients whenever the database changes, and the client reconnects automatically when the connection is closed.
...
Because we use [Prisma](https://www.prisma.io/) as our ORM, we can use [Prisma Extensions](https://www.prisma.io/docs/orm/prisma-client/client-extensions) to hook into database operations and write events to Redis. This is where the `DatabaseService` mentioned in the [Endpoints](./endpoints) comes into play.

> [!IMPORTANT]
>
> The existing implementation has the folowing cons:
> - Deletions need to be explicit, even if cascade deletions are handled automatically by the database. See [`UserController.deleteUser`](https://github.com/finom/realtime-kanban/blob/main/src/modules/user/UserService.ts) method for more details.
> - All write operations must select the `updatedAt` field for the change detection to work properly.
> - List of supported write operations is limited to `create`, `update`, `upsert`, and `delete` for simplicity. Read operations are passed through as-is. For more complex operations, additional handling or abstraction is required.
...
The `delete` operation also adds an `__isDeleted` property. The front end checks this property to hide the deleted entity by setting `enumerable: false` on the entity registry item (see the [State](./state) page).
...
From now on, when the database is changed by other users or third-party services, the front end will receive updates in real time and the entity registry will be updated accordingly, keeping the UI in sync with the back-end data.