...
Because we use [Prisma](https://www.prisma.io/) as our ORM, we can use [Prisma Extensions](https://www.prisma.io/docs/orm/prisma-client/client-extensions) to hook into database operations and write events to Redis. This is where the `DatabaseService` mentioned in the [previous article](./endpoints) comes into play.

> [!IMPORTANT]
>
> A downside of this approach is that the deletions need to be explicit, even if cascade deletions are handled automatically by the database. See [`UserController.deleteUser`](https://github.com/finom/realtime-kanban/blob/main/src/modules/user/UserService.ts) method for more details.

```ts showLineNumbers copy filename="src/modules/database/DatabaseService.ts" repository="finom/realtime-kanban" {20, 114}
...
export default class DatabaseService {
  ...
  private static getClient() {
    ...
    return prisma
      .$extends({
        name: "timestamps",
        // Ensure createdAt and updatedAt are always ISO strings to match Zod schemas
        result: {
          $allModels: {
            createdAt: {
