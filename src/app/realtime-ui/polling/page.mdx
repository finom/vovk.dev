...
[State normalization](./state), a custom [fetcher](./fetcher), and the corresponding backend implementation keep the UI updated while the user interacts with the app by making HTTP requests. The implementation also covers AI features such as [Text Chat AI](./text-ai) and [Voice AI](/realtime-ui/voice-ai), explained in later articles. The rule of thumb is that data should always be processed by the entity registry.
