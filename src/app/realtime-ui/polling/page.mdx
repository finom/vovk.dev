...
import GithubFiles from '@/components/github-files';
...
<img src="/diagrams/collaborative_polling_flow.svg" alt="Collaborative Polling Flow" className="mt-4 dark:invert" />
...
On the client side (for example, in a React component), call `DatabasePollRPC.poll()` to receive a stream of database events. As with any [JSONLines](/jsonlines) RPC method, it returns an async iterable that you can consume in a `for await` loop. Because the server may close the connection or a network error may occur, wrap the logic in a retry loop. Since the [fetcher](/imports#fetcher) is already [configured](./state#setting-up-the-fetcher), the loop body can be empty—you do not need to handle data manually.
