...
A combination of three features, such as [State Normalization](./state), [Database Polling](./polling) and [Tool derivation](/tools) enables RealTime database and UI updates from any third-party source of changes: API calls, MCP clients, bots, interactions by other users, etc.

As a proof of concept, this article will briefly describe the Telegram bot integration, that allows users to create tasks and assign them to team members by sending text or voice messages to the bot.

## Relegram API Mixin

The Telegram API library is implemented with [OpenAPI mixins](/mixins) and used as a module with hard-coded name `TelegramAPI` in the `telegram` pseudo-segment.
...
After running `vovk dev` or `vovk generate`, the `TelegramAPI` module will be available for import from the generated client library.
...
As the Telegram bot API requires authentication via bot token in the URL, the API module can be recreated with `withDefaults` method to set the `apiRoot` permanently.
...
We're going to create a new segment called `bots`, that will contain a controller `TelegramController` under `TelegramBot` key. The segment isn't going to emit any schema as it's not needed on the client side by setting `emitSchema: false` option in the `initSegment` call. Since the schema is not emitted, the key name of the controller can be set to any string.
...
The `TelegramController` class is going to contain a single `handle` procedure, implementing `/api/bots/telegram/bot` endpoint, that will be called by Telegram webhook on each incoming message.
...
The `TelegramService` class contains the main logic of handling incoming messages, generating AI responses with Vercel AI SDK, updating the database via derived tools, and sending text or voice messages back to the user.
...
Now the app is ready to receive incoming messages from Telegram users, process them with AI, update the database and UI, and respond back to the users.