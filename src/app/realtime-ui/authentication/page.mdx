...
but also exports `isLoggedIn` function that is created for `sessionGuard` [decorator](/authorization) to check if the user is logged in when invoking controller methods.
...
The `sessionGuard` decorator applied to all controller methods. `typeof req.url !== 'undefined'{:ts}` check is required to distinguish between HTTP requests and [`fn`](/controller/fn) invocations used by PPR and [Text Chat AI](/realtime-ui/text-ai) features.
