...
The `sessionGuard` decorator applied to all controller methods. `typeof req.url !== 'undefined'{:ts}` check is required to distinguish between HTTP requests and [`fn`](/fn) invocations used by PPR and [Text Chat AI](/realtime-ui/text-ai) features.
