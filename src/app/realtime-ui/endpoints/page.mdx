...
- Each method is decorated with the [@operation](/openapi) decorator that describes the operation, including `summary` and `description`.
- Each method is created with the [procedure](/procedure) function that implements request validation and schema emission.
...
- The controller procedures use [req.vovk](/procedure#req-vovk-interface) to access request data such as `params`, `query`, and `body`, because we want to call these methods directly from code (not only through HTTP requests) via the [fn](/fn) interface for SSR/PPR and AI tool execution.
