...
As we're going to use [function calling](/function-calling) we're going to change the `target` of the generated JSON schemas to `draft-7`, as LLMs currently support only this version.
...
The [controllers](/controller) are going to declare full CRUD operations for `User` and `Task` entities, implemented as follows:

- Each method is decorated with [@operation](/openapi) decorator that describes the operation, including `summary`, `description`, and custom `x-tool-successMessage` and `x-tool-errorMessage` properties for [MCP](./mcp) responses.
...
The [services](/controller/service) are going to implement the actual business logic for each controller method, including database operations using Prisma client and embedding generation using OpenAI embeddings.
