...
Request overhead testing measures the framework's impact on individual request latency. Since Vovk.ts builds upon Next.js's native routing capabilities, our goal is to isolate and measure the additional processing time introduced by the framework's decorator system, parameter validation, and routing logic.
...
This design ensures realistic overhead measurement by including:
- Decorator metadata processing.
- Route pattern matching for parameterized paths.
