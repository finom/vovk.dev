...
`req.vovk.body()` automatically parses the form data into a plain object (see [`req.vovk` Interface](#req-vovk-interface)). You can also access the raw `FormData{:ts}` instance via `req.formData()`.
