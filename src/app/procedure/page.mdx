...
Procedure in Vovk.ts defines a RESTful API endpoint handler. It's defined as a static method of a class, called "controller", decorated with an HTTP method decorator like `@get()`, `@post()`, `@del()`, `@put()`, `@patch()`. A procedure is a wrapper around Next.js API route handler that accepts `NextRequest` object and parameters defined in the route path.
...
See [local procedures](/fn) for details.
