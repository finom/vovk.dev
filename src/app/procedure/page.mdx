...
The function returns an object with `.handle()` method that accepts an async function that processes the request after validation succeeds. It receives a type-enhanced `NextRequest{:ts}` as `VovkRequest<TBody, TQuery, TParams>{:ts}` and the `params: TParams{:ts}` value as the second argument.
