...
TODO - Client method accepts `fetcher` and `validateOnClient` options. TODO handler.schema property
...
As you can see all the files export one variable per controller and their functionality is built from the [schema](/schema) and types exported from **route.ts** file, making the client library type-safe and very compact in size for production use.
