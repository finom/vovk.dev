...
1. It generates a client into the `tmp_prebundle` directory (configured with `bundle.prebundleOutDir: string{:ts}`) using the [ts](/templates#ts) template.
2. Uses the [tsdown API](https://tsdown.dev/) to bundle the generated client into a few compiled files in the `dist` directory.
...
Any options that can be passed to the [tsdown](https://tsdown.dev/) `build` function. The options are outlined in the [tsdown documentation](https://tsdown.ru/reference/config-options).

### `generatorConfig`

The `generatorConfig` object accepts and overrides the same options as the [generatorConfig](/config#generatorconfig) at the root of the [config](/config) file.
