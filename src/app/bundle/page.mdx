...
The [composed](/composed) [TypeScript Client](/typescript) library can be bundled and published to NPM with pre-filled `package.json` and `README.md` files using the `bundle` command after setting up `bundle.build` function in the [config](/config) file.
