...
Note that in order to be able to use safe self-inferences at [service classes](/controller/service), the `handle` method doesn't force the return type, being able to return any value. However, you can use `satisfies` operator to make sure that the returned value matches the expected output type.
