...
A controller module is a static class (a class that has only static methods and is never instantiated) that handles incoming HTTP requests.

Methods of this class that are decorated with an HTTP decorator (like `@get()` or `@post()`) become request handlers and accept 2 arguments: [VovkRequest](/controllers/request) (which is inherited from `NextRequest`) and parameters that are set by the decorator path.
...
To add the required body and query types, you need to replace `NextRequest` with `VovkRequest` and define the types as generic arguments.
...
As you can see, we've changed nothing more than the type of `req`, but now `data` receives the type of `Partial<User>` and `notify` is cast as `'email' | 'push' | 'none'` and no longer extends `null`.
...
Note that to enable safe self-inference in [service modules](/controller/service), the `handle` method doesn't enforce the return type and can return any value. However, you can use the `satisfies` operator to ensure that the returned value matches the expected output type.
...
All HTTP decorators provide an `.auto` method that generates the endpoint name automatically from the method name, making the handler definition more RPC-like.
