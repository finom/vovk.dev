...
Once the controller is defined, it needs to be initialized in a [segment route](/segment) by adding it to the `controllers` object. The key of this object defines the name of the resulting [RPC module](/rpc-module) variable exported from `"vovk-client"` or the [segmented client](/segmented).
