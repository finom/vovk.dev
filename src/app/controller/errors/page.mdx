...
You can gracefully throw HTTP exceptions using syntax inspired by NestJS. The `HttpException` class accepts three arguments: an HTTP code from `HttpStatus`, a message, and an optional `cause` object. This object is JSON‑stringified and sent to the client.
...
You can also throw custom objects that are rethrown on the client side as is.
