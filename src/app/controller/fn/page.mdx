...
export default class UserController {
  ...
  static updateUser = withZod({
    ...
    query: z.object({ /* ... */ }),
    output: z.object({ /* ... */ }),
    async handle({ vovk }) { // destructuring req to get "vovk"
...
- **For SSR, SSG, PPR etc**: You can use the method as a server-side function, allowing you to call it from the server-side code.
