...
Vovk.ts provides a way to pass additional metadata to controller methods, which can be useful for various purposes such as logging, analytics, or custom processing. The main purpose of this feature is to set request metadata in a custom decorator, which can be used later in the controller handler or in other decorators declared after the one that sets the metadata.
...
The metadata is a key-value object that is extended by passing an object to the `vovk.meta` method. When `vovk.meta` is called multiple times with different keys, the metadata object is merged, so you can set multiple metadata keys in one call.
...
To reset the metadata, you can call `req.vovk.meta(null)`, which will clear the metadata object.
...
The compiled RPC module handlers can send custom underlying request metadata to the server, which is transferred as a JSON string in the `x-meta` header. This is useful when you want to pass additional information from the client side to the server side, such as user preferences, session data, or any other custom data relevant to the request.
...
The `getUser` controller method, as well as the [decorators](./decorator), can access this metadata through the `req.vovk.meta` method under the `xMetaHeader` key:
...
This design protects server-side metadata from being overwritten by client-side code, as it is only accessible through the `xMetaHeader` key.
