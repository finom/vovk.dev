...
> [!IMPORTANT]
>
> As response size is not known in advance, JSON Lines responses cannot be compressed with Gzip, Brotli or other algorithms. Make sure to consider this when using JSON Lines for large responses.
...
The iterable, (represented as `stream` variable in the example above) besides `Symbol.asyncIterator`, `Symbol.dispose`, and `Symbol.asyncDispose` methods, also provides the following members:

- `status`: The status of the HTTP response (e.g. 200 for OK, 404 for Not Found).
- `asPromise`: Returns a promise that resolves with an array of all emitted values when the stream is completed.
- `onIterate`: Registers a callback to be called on each iteration.
- `abortController`: An `AbortController` instance that can be used to abort the stream. When stream is closed with `abortController.abort(){:ts}`, it throws an `AbortError`.

The `using` statement ensures that the stream is aborted using `stream.abortController.abort('Stream disposed'){:ts}` when it goes out of variable scope.
