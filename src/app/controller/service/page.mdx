...
Service is a part of the Controller-Service-Repository pattern that is used to separate business logic from the request handler. It allows to keep your controllers clean and focused on handling HTTP requests, while the service class contains the actual business logic and data manipulation.
...
The best thing about service methods is that their typing can be inferred from controller methods that call them, without causing "implicit any" TS errors. Thanks to [Anders Hejlsberg](https://github.com/ahejlsberg) for a solution for [#58616](https://github.com/microsoft/TypeScript/issues/58616), without this fix to TypeScript language Vovk.ts wouldn't be implementable.
