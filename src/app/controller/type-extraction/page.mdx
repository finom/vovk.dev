...
Note that `VovkControllerOutput` and `VovkControllerReturnType` are not the same as well as `VovkControllerYieldType` and `VovkControllerIteration`. `VovkControllerOutput` and `VovkControllerIteration` infer types assigned at validation libraries. These types are created as a workaround in order to be able to avoid circular reference errors (implicit "any") when controller method uses types of a service method and vice versa. Read more described in the [validation documentation](/validation).
