...
But what if you don't know what chunk will come first, second and so on? In this case, you can use an experimental feature that I call "progressive response", inspired by Dan's Abramov's proposal [Progressive JSON](https://overreacted.io/progressive-json/), where I borrowed the "progressive" name from.
...
Let's say you have two functions that return data after some random delay: `getUsers` and `getTasks`, implemented as static methods of a [service class](./service). In a real application, these could be API calls or queries to different databases; they also can be connected to each other by ID, for example, each task can have a user ID that it belongs to, but for this example, we will keep them independent.

With the help of [JSONLinesResponse class](./jsonlines#jsonlinesresponse), we can implement a simple method, whose implementation will look like this:
