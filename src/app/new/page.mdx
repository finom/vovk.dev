...
The command `vovk new` allows to create new [segments](/segment) and modules (such as [controllers](/controller), [services](/service), or custom modules) in the project. It uses `moduleTemplates` option defined at [config](/config) and can be extended with custom templates. It includes two commands in one: `vovk new segment [segment_name]` and <nobr>`vovk new [module] [module_name_singular]`</nobr>.
...
When `vovk new segment foo/bar/baz` is run, it creates a nested segment represented as **/src/app/api/foo/bar/baz/[[...vovk]]/route.ts** file, available as **/api/foo/bar/baz/...** endpoint. `segmentName` option equals to `"foo/bar/baz"`.
...
[vovk init](/init) initializes the Vovk.ts project with the corresponding templates defined at the [config](/config) file.
