...
The `vovk generate` command creates [TypeScript](/typescript), [Rust](/rust), and [Python](/python) clients from existing [schema](/schema) files and from [OpenAPI mixins](/mixins) declared in the [config](/config). Relevant configuration spans several options and documentation pages, including:
...
- `outputConfig`: options to configure `origin`, [imports](/imports), [OpenAPI mixins](/mixins), and more.
...
Mixins extend the sclient declared in one or more OpenAPI specs. See [OpenAPI mixins](/mixins).
