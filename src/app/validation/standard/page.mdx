...
All other libraries are implemented in a similar way, the only difference might be `toJSONSchema` implementations. Once [standard-schema/issues/21](https://github.com/standard-schema/standard-schema/issues/21) is resolved, the requirement to use `createStandardValidation` will probably become obsolete in most of cases.
