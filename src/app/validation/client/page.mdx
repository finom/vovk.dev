...
The field `config.imports.validateOnClient` is used to specify the path to the `validateOnClient` function that is imported by the generated TypeScript client and passed to `createRPC` function that initializes an RPC module.

The library can be configured by adjusting `config.libs.ajv` field in the `vovk.config.mjs` file to set Ajv options, localization and target version of the JSON schema. The type `VovkAjvConfig` should be set explicitly since `vovk-ajv` is a separate package and the type is not imported automatically.
...
<Tabs items={['npm', 'yarn', 'pnpm']}>
  <Tabs.Tab>```bash npm install vovk-ajv ```</Tabs.Tab>
  <Tabs.Tab>```bash yarn add vovk-ajv ```</Tabs.Tab>
  <Tabs.Tab>```bash pnpm add vovk-ajv ```</Tabs.Tab>
</Tabs>
...
### DTO client-side validation and transform
...
If you want the response to be transformed to a DTO class, you can use `transform` option described at [TypeScript client documentation](/typescript).

```ts
import { UserRPC } from 'vovk-client';
import { plainToInstance } from 'class-transformer';
import { UpdateUserOutputDTO } from './UserDTO';

const response = await UserRPC.updateUser({
  // ...
  transform: (data) => plainToInstance(UpdateUserOutputDTO, data),
});

console.log(response instanceof UpdateUserOutputDTO); // true
```
...
> [!WARNING]
>
> By the time being `FormData` is **not validated** and **does not emit JSON schema** (you can provide it manually at [openapi decorator](/openapi)) due to unsolved library design issues. This problem has high priority and will be fixed as soon as a good solution is found. If you want to help to brainstorm the solution that allows not just validate the data but also emit schema (that can be converted into OpenAPI schema), please create an issue. For now you can create a [custom decorator](/controller/decorator) that might look like `@validateFormData(validateFn, multipartFormDataOpenAPISchema)` that performs validation at the first argument and extends handler's `openapi` schema with the second argument.

In order to make the RPC method to accept form data, you need to define the body type as `FormData` (see [the controller docs](/controller/form-data)) and set the body validation model to `null`.

```ts filename="src/modules/user/UserController.ts" {8}
import { prefix, post } from 'vovk';
import { withZod } from 'vovk-zod';
import { z } from 'zod';
...
export default class UserController {
  @openapi({
    summary: 'Update user',
    description: 'Update user by ID with form data',
    requestBody: {
      content: {
        'multipart/form-data': {
          schema: {
  ...
  @post('form', {
    before: (req) => req.formData(), // TODO
  })
  static updateUser = withZod({
    body: null as unknown as z.ZodType<FormData>, // doesn't look good
    async handle(req) {
...
Since it looks quite bulky, every validation library provides `formData` property that does the same but makes the code look nicer.
...
// ...
export const { GET, POST, PATCH, PUT } = initSegment({
  controllers,
...
Both Yup and Zod have built-in utilities to turn one model into another using `pick`, `omit`, `partial`, etc. **class-validator** in its turn requires to install [@nestjs/mapped-types](https://www.npmjs.com/package/@nestjs/mapped-types) to use the same functionality. Since the package has peer dependencies that are unnesessary but also break the build, you can use its fork [dto-mapped-types](https://github.com/finom/dto-mapped-types) instead that provides the same functionality in Edge Runtime but without **@nestjs/common** dependency. For more information see the [article about the original package](https://trilon.io/blog/introducing-mapped-types-for-nestjs).
