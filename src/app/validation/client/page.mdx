...
As there is no standard way to describe `FormData` in JSON Schema, every validation library have to provide its own way to generate OAS-compatible schema for `FormData` requests.
...
Zod 4 includes `.file()` schema that is converted to the valid [OAS schema](https://zod.dev/json-schema?id=file-schemas) but for other libraries it needs to be solved individually. It's outlined for every library mentioned at this documentation (for arktype it's solved by using a `fallback`, for Valibot you need to use `overrideSchema` etc).

---

In case if you use a library that isn't mentioned here and you don't know how to make it properly generate the OAS file schema, the easiest way to work around this problem is to use `disableClientValidation` option in the generated RPC modules:
