...
The `schema` property provides the method schema for the handler, similar to the RPC method schema. It's used in tendem with the `fn` property to build [Function Calling tools](/ai-tools) to call the handler directly from the AI tools, without making an HTTP request.
