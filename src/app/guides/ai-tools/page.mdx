...
Vovk.ts supports 3rd-party APIs, that expose their OpenAPI specification, so you can mix your own back-end functions with 3rd-party APIs in a single LLM Function Calling interface. See [Codegen guide](/guides/codegen) for more details.
...
```ts filename="src/app/guides/ai-tools/page.tsx"
...
An [LLM guide](/guides/llm) provides more details on how to use `useChat` hook. Also check the [Vercel AI SDK documentation](https://ai-sdk.dev/docs/reference/ai-sdk-ui/use-chat) for more details on how to use the hook.
