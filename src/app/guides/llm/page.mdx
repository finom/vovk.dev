...
LLM completions can be streamed using [JSON Lines](https://jsonlines.org/) format, which is a convenient way to send a stream of JSON objects over HTTP. Vovk.ts provides a `JSONLinesResponse` class that can be used to create such responses.
