...
Each segment emits its backend schema to an individual JSON file, which is then used to generate client-side [RPC modules](/rpc-module). If your application has multiple areas (e.g., root, admin, customer), each area has its own schema file.
