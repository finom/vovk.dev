...
`_meta.json` contains additional metadata, including selected fields from [vovk.config](/config) under the `config` key.
...
The scmela is available as the `schema` export.
