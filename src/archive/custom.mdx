...
TODO: This is the old version of the documentation. It needs to be updated.
...
```ts showLineNumbers copy filename="src/decorators/validateEquality.ts"
import { isEqual } from 'lodash';
import { HttpException, HttpStatus, createDecorator, type VovkRequest, type VovkClientOptions } from 'vovk';
...
```ts showLineNumbers copy filename="src/decorators/validateEqualityOnClient.ts"
...
In order to enable client-side validation you need to define `validateOnClient` option in **vovk.config.js** file. For more info see [customization documentation](/imports).
